...
k
#%%
moving_average_numpy(np.log(Y[4003][k:j]),1)
#%%
...
#%%
Y2 = Y
trial_numbers2 = trial_numbers
larmor_frequencies2 = larmor_frequencies
times2 = times
conductivity2 = conductivity
turbidity2 = turbidity
temperature2 = temperature
signal_limits2 = signal_limits 
concentrations2 = concentrations
pH2 = pH
#%%
Y = Y2
larmor_frequencies = larmor_frequencies2
times = times2
conductivity = conductivity2
turbidity = turbidity2
temperature = temperature2
signal_limits = signal_limits2
...
len(signal_limits)
#%%
plt.plot(np.arange(0,len(parameters)),parameters,linewidth=0.5)
plt.show()
#%%
plt.scatter(times,conductivity)
plt.show()
#%%
limits = []
i = 0
...
#%%
#concentrations = concentrations[0:len(parameters)-len(concentrations1)]

concentrations1 = sorted(concentrations1)
for i in concentrations1:
    concentration.append(float(i)/1000)
#%%
parameters[4400:4500]
#%%
...
plt.show()
#%%
plt.plot(np.arange(0,len(con)),con)
plt.show()
#%%
...
for i in Y[0:4320]:
    ...
    j = j + 1
#%%
plt.plot(np.arange(0,len(parameters)),std)
plt.show()
#%%
j = 0
k = []
...
rf = clf.best_estimator_
#%%
#cross_validation(rf, X, concentrations, kf)
...
plt.savefig('system_validation.png', bbox_inches='tight',dpi=300)
#%%
columns=['Mean' ,'Amplitude' ,'RMS' ,'Kurtosis', 'Standard Deviation', 'Shape Factor', 
          'Skewness', 'Crest Factor', 'Impulse Factor', 'Larmor Frequency', 'Water Temperature',
         'pH', 'Turbidity', 'conductivity', 'T2']
df = pd.DataFrame(X, columns=['Mean' ,'Amplitude' ,'RMS' ,'Kurtosis', 'Standard Deviation', 'Shape Factor', 
          'Skewness', 'Crest Factor', 'Impulse Factor', 'Larmor Frequency', 'Water Temperature',
         'pH', 'Turbidity', 'conductivity','T2'])
plt.plot(np.arange(len(Y)), concentration, label='concentration')
for i in columns:
    plt.plot(np.arange(len(Y)), df[i], label=i)
plt.legend(bbox_to_anchor=(1.0, 1), loc='upper left')
plt.xlabel('Trial Number')
plt.ylabel('Parameter Value (normalized)')
plt.savefig('partial_dependence.png', bbox_inches='tight',dpi=300)
#%%
...
plt.show()
#%%
Z[12][1159]-Z[12][1158]+TIMES[3627]
#%%
import matplotlib.pyplot as plt
plt.rcParams.update({'font.size': 9})  # increase the font size
plt.rcParams['font.family'] = 'Serif'
...
plt.show()
#%%
plt.plot(np.arange(0,len(mean)),std)
plt.show()
#%%
np.argmax(mean[0:2000])
#%%
signal_limits[503]
#%%
...
#%%
plt.plot(np.arange(0,len(parameters)),parameters)
plt.show()
#%%
j = 0
k = []
...
for i in Y:
    ...
    j = j + 1
#%%
plt.plot(np.arange(0,len(mean)),mean)
plt.show()
#%%
j = 0
k = []
